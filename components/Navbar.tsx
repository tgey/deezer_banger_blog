...
import { ReactComponent as SvgLogo } from 'public/maison.svg';
    ...
    <NavbarContainer hidden={isNavbarHidden}>
      <Content>
        <NextLink href="/" legacyBehavior passHref>
          <LogoWrapper>
          {/* <NavItem key={home_button.href} {...home_button} /> */}
...
const NavItemWrapper = styled.li<Partial<SingleNavItem>>`
  ...
  line-height: 2;
  border: 1px solid rgb(var(--secondBackground));
  // box-shadow: ${(p) => (p.outlined ? '4px 4px 0px 0px rgb(var(--secondBackground))' : 'none')};

  &:hover {
    transition: background-color 0.2s;
    background-color: ${(p) => (p.outlined ? 'rgb(var(--primary), 0.8)' : 'transparent')};
    transform: ${(p) => (p.outlined ? 'translateX(3px) translateY(3px)' : 'none')} ;
    box-shadow: none;
  }

  a {
    display: flex;
    color: ${(p) => (p.outlined ? 'rgb(var(--background))' : 'rgb(var(--text), 0.75)')};
